  ...
  return (
    <div className="container mt-4">
      ...
      <div className="row row-cols-1 row-cols-md-3 g-4 mt-4">
        {categories.length > 0 ? (
          categories
            .filter((category) => !category.parent) // Show only main categories
            .map((category) => (
              <div key={category.id} className="col">
                <div className="card shadow-lg border-0 h-100 d-flex flex-column">
                  <div className="image-container overflow-hidden text-center p-5">
                    <img src={`https://127.0.0.1:8000/uploads/categories/${category.photo}`} alt={category.nom}className="card-img-top img-fluid" 
